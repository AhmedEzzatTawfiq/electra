...
const Banner = async () => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-10 md:max-h-[600px]">
      ...
      <div className="md:col-span-2 shadow-sm shadow-lightGreen relative flex items-end justify-end rounded-lg overflow-hidden group">
      ...
      <div className="flex flex-col space-y-5 md:space-y-10 h-auto md:max-h-[600px]">
        {banners.slice(1, 3).map((item: BannerData) => (
          <div
            key={item._id}
            className="h-full md:h-1/2 shadow-sm shadow-lightGreen rounded-lg overflow-hidden flex justify-center items-center p-5 group"
          >
